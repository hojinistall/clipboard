...
def capture_to_area(diallog):
    ...
    def on_left_button_up(event):
        ...
        try:
            ...
            strPosition = "캡쳐영역 ["+str(x_start) + "," + str(y_start) + \
                "," + str(x_end) + "," + str(y_end) + " ]"
...
def makeAlertLabel(root, msg):
    for widget in root.winfo_children():
        if isinstance(widget, tk.Label) and (widget['fg'] == 'red' or widget['fg'] == 'blue'):
            widget.destroy()
    lbl_text = tk.Label(root, text=msg, fg="red", font=("맑은 고딕", 1))
...
def makeProcessLabel(root, msg):
    for widget in root.winfo_children():
        if isinstance(widget, tk.Label) and (widget['fg'] == 'red' or widget['fg'] == 'blue'):
            widget.destroy()
    lbl_text = tk.Label(root, text=msg, fg="blue", font=("맑은 고딕", 1))
    root.after(300, lambda: lbl_text.place(
        relx=0.5, rely=0.9, anchor="center"))
    #lbl_text.place(relx=0.5, rely=0.9, anchor="center")
...
def makePositionLabel(root, msg):
    for widget in root.winfo_children():
        if isinstance(widget, tk.Label) and widget['fg'] == 'green':
            widget.destroy()
    lbl_text = tk.Label(root, text=msg, fg="green", font=("맑은 고딕", 1))
...
# DPI 설정
root.tk.call('tk', 'scaling', 4.0)  # DPI를 2.0으로 설정합니다.

# 윈도우 크기 설정
window_width = int(root.winfo_screenwidth() * 0.1)
...
lbl_text = tk.Label(root, text="© PAULTALL", fg="black", font=("", 2))
...
btn_create_capture = tk.Button(
    root, text="영역지정", command=lambda: capture_to_area(root), font=("", 7))
btn_create_capture.place(relx=0.5, rely=0.15, anchor="center")

# 캡쳐 영역 생성
btn_create_capture = tk.Button(
    root, text="클립보드", command=capture_to_clipboard, font=("", 7))
